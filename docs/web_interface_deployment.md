...
## 8. Діагностика UI: чому постійно підсвічується перший елемент списку

Якщо в інтерфейсі «Керування довідниками» (або аналогічному списку) візуально завжди виділений перший елемент,
причина зазвичай у фронтенд-стані, а не в бекенді API:

1. **Стан активного елемента ініціалізований як `0` або `items[0].id` і не оновлюється по кліку.**
   Перевірте, чи обробник кліку справді викликає `setActiveId(...)`/`dispatch(...)`.
2. **Порівняння ідентифікаторів різних типів (`string` vs `number`).**
   У такому разі умова `activeId === item.id` хибна для всіх, крім значення за замовчуванням.
3. **Статичний CSS-клас `active`/`selected` на першій картці.**
   Переконайтеся, що клас додається лише динамічно: `item.id === activeId`.
4. **Ключі списку (`key`) неунікальні або прив’язані до індексу.**
   При ререндері це може «приклеювати» активний стиль до першого блоку.

Швидка перевірка: тимчасово виведіть у UI `activeId` і `item.id`, а також перевірте, чи
змінюється `activeId` після кліку. Якщо значення не оновлюється — причина в обробнику події;
якщо оновлюється, але стиль не змінюється — причина в умові класу або в CSS-специфічності.
