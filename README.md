...
через веб-інтерфейс. Для старту потрібно:

- визначити вимоги до користувацького сценарію (які фільтри та опції необхідні);
- описати API або бекенд-скрипт, що формуватиме вибірковий файл;
- підготувати макет інтерфейсу та перевірити його на наявному середовищі Pi-hole;
- оцінити вплив на поточні скрипти `generate_whitelist.sh` та `apply_whitelist.sh`.

Після цього можна переходити до наступних пунктів квартальної дорожньої карти.
