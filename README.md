...
python3 whitelist_builder_api.py --host 127.0.0.1 --port 5050 \
  --api-token "замініть-на-надійний-токен" \
  --data-dir /tmp/whitelists --log-file /tmp/whitelist_builder.log
```
Основні параметри можна змінювати: `--categories-dir` (каталог з файлами категорій), `--allow-extra-path` (дозволені додаткові каталоги), `--allow-external-categories` (щоб приймати абсолютні шляхи файлів), `--allow-apply-directly` (явно дозволити застосування через Pi-hole з API).  
API-токен обов'язковий: передайте `--api-token` або змінну `WHITELIST_BUILDER_API_TOKEN`.
...
curl -X POST http://127.0.0.1:5050/api/build \
  -H 'Authorization: Bearer замініть-на-надійний-токен' \
